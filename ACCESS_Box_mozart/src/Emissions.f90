...
module Emissions
  ...
  private CalcRawEmissions, MapRaw2Mech, gam_t, gam_t_lif, gam_t_ldf, gam_l, gam_l_ldf
  public GetEmissions
...
!***********************************************************************************************************************!
subroutine MapRaw2Mech()
  integer(kind=i4)  :: iz

  ! map to explicit species in mechanism mozart
  do iz=1,npts
    q(iz,    iC5H8)  = qraw(iz,bisop)+qraw(iz,bmbo)
    q(iz,    iHCHO)  = qraw(iz,bhcho)
    q(iz,      iCO)  = qraw(iz,bco)
    q(iz,    iC2H6)  = qraw(iz,betha)
    q(iz,    iC2H4)  = qraw(iz,bethe)
    q(iz,  iCH3CHO)  = qraw(iz,baald)
    q(iz,iCH3COCH3)  = qraw(iz,bacet)
    q(iz,    iTERP)  = qraw(iz,bmyrc)+qraw(iz,blimo)+qraw(iz,bocim)+qraw(iz,bpcym) &
                 +qraw(iz,bocym)+qraw(iz,batrp)+qraw(iz,bgtrp)+qraw(iz,bbphe) &
                 +qraw(iz,bafrn)+qraw(iz,bbcro)+qraw(iz,bsabi)+qraw(iz,bcare) &
                 +qraw(iz,bapin)+qraw(iz,bbpin)
  end do
