...
subroutine MapRaw2Mech()
  ...
  ! map to explicit species in mechanism cbmz
  do iz=1,npts
    q(iz, iISOP)  = qraw(iz,bisop)+qraw(iz,bmbo)
    q(iz, iHCHO)  = qraw(iz,bhcho)
    q(iz,   iCO)  = qraw(iz,bco)
    q(iz, iC2H6)  = qraw(iz,betha)
    q(iz,iCH3OH)  = qraw(iz,bmeoh)
    q(iz, iALD2)  = qraw(iz,baald)
    q(iz, iAONE)  = qraw(iz,bacet)
    q(iz, iC2H4)  = qraw(iz,bethe)
    q(iz, iOLET)  = qraw(iz,bbute)
    q(iz,iHCOOH)  = qraw(iz,bform)
    q(iz,iRCOOH)  = qraw(iz,bacta)
  end do
